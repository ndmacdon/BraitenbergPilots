...
// Defines a sensor returning a double:
...
// Casts a ray from <l> in direction <r> and returns minimum distance to any
// intersected object:
...
/*
  Represents a 'socket' attached to a vehicle, may have a sensor attached.
*/
class HardPoint {
  Vector2D locationOffset;
...
class BraitenbergVehicle {
  ...
  // Read output from hardpoint sensors and submit it into thresholds:
  void sense() {
  ...
  void signal() {
    ...
    for (Wire w : controlWires) {
      ...
      System.out.println(String.format("threshResult: %,3.2f", thresholdResult));
  ...
  // Reset all controlSignals and threshold-device charges:
  // Avoids seizure-states.
  void relax() {
    for (DoubleThreshold t : dThresholds) {
      t.reset();
    }
...
/*
  Very Basic BraitenBerg Vehicle used for testing Sensors/Signals/Thresholds.
  Can sense the gamestate and generate command signals.
*/
class BraitenbergCockroach extends BraitenbergVehicle {
  ...
  void update() {
    ...
    sense();
    signal();
    System.out.println(String.format("signal[0]: %,3.2f", controlSignals[0]));
